...
# A* alg
def find_path_alg(img,img2, width, height, starting_position, ending_position, path_color):
    ...
    while len(open_list) != 0:
        ...
        if is_destination(img, cur_x, cur_y, ending_position):
            print_path(img2, starting_position, ending_position, path_color, node_details)
            return
...
def index():
    # global variables
    img = cv2.imread("static/floor2-line.png")
    img2 = cv2.imread("static/floor2.png")

    width = int
    height = int
    starting_position = list()
    ending_position = list()
    path_color = (227, 29, 29)

    dimensions = img.shape
    height = dimensions[0]
    width = dimensions[1]
    # print(width+"height"+height)
    starting_position.append(272)
    starting_position.append(69)
    ending_position.append(169) #x
    ending_position.append(271) #y

    find_path_alg(img,img2, width, height, starting_position, ending_position, path_color)

    cv2.imwrite("static/new.png", img2)
    

    return render_template('select_room.html', shortest_path_img=img2)
